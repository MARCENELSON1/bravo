...
- **C — Llamar al mozo / pedir la cuenta: ✅ HECHA.** Use case `RequestTableAttention` (verifica token → publica `DomainEvent(type="floor.call", payload={table_id, table_number, kind})` en el bus existente, reusa la infra SSE del KDS/floor). Endpoints públicos `POST /public/table/call-waiter` + `/request-bill`. Front: `useRealtimeEvent` (generalización de `useRealtimeInvalidate` que expone el payload) → el floor muestra un toast "Mesa N te llama / pide la cuenta" (8s); barra de acciones fija en la carta con toasts de confirmación. i18n `publicMenu.actions/toast` + `floor.toast.callWaiter/bill` (ES/EN). Gates: back **621 tests** + ruff · front build+lint+**215 tests**. Nota: rate-limit del endpoint diferido (bajo riesgo en F1; solo notifica).
- **D — Gestión/impresión de QR (lado dueño): ✅ HECHA.** Página full-screen `/app/mesas-qr` (fuera del shell, OWNER/MANAGER) para imprimir limpio (`print:hidden` en la barra, `print:break-inside-avoid` por tarjeta). Lista las mesas activas y por cada una dibuja el QR en el cliente con **`qrcode.react`** (`QRCodeSVG`, fondo blanco siempre para que escanee) desde `GET /tables/{id}/qr` (endpoint ya existente de la Tanda A) vía `useTableQr`. Ítem de nav "QR de mesas" (Gestión, icono `ScanLine`) + i18n `shell.nav.tableQr` + `floor.qr.*` (ES/EN). Gate: front build+lint+**216 tests** (1 nuevo). Sin backend nuevo.

Cada tanda cierra con sus gates. A+B ya dejan la carta QR en la calle; C+D la completan.

## ✅ FASE 1 COMPLETA (A–D)
La carta QR está de punta a punta: el dueño imprime los QR (D), el comensal escanea → ve la carta (A+B) y puede llamar al mozo / pedir la cuenta (C), que llega al floor en vivo. Todo en `main` y pusheado. Pendiente no-bloqueante: chequeo visual en navegador (carta claro/oscuro + impresión de QR). Próxima: **Fase 2 (autopedido)** — reusa la sesión de mesa y agrega carrito + enriquecimiento del producto (fotos/descr./modificadores/disponibilidad).
