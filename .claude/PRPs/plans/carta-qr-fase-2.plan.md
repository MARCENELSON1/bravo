...
- **D — Modificadores (backend): ✅ HECHA** (`09575d2`). `ModifierGroup`+`ModifierOption` (`domain/product/modifiers.py`) con validación de reglas (`InvalidModifierGroup`) + resolver puro `select_options()` (min/max + opción desconocida → `InvalidModifierSelection`). **Precio server-side:** el `price_delta` se **pliega en `unit_price`** del `OrderItem` (toda la matemática sale_facts/finanzas/factura sigue leyendo UN número) y las opciones quedan como **snapshot display-only** (`SelectedOption`) para el ticket. `SubmitCustomerOrder` resuelve las opciones elegidas contra el catálogo (el carrito solo manda `option_ids`) y valida min/max antes de crear. Carta pública expone los grupos (id+name+price_delta+required). CRUD dueño `GET/PUT /products/{id}/modifiers` (OWNER/MANAGER, replace-all, ids server-side). Tablas `product_modifier_groups`+`product_modifier_options` (RLS) + JSON `order_items.selected_options`. Migr. **0048**. El order response incluye `selected_options` (para floor/KDS de Tanda E). Todo aditivo/nullable → **paridad** (producto sin modificadores + flujo del mozo idénticos). Gates: back **648 tests**+ruff.
- **E — Modificadores + config (frontend): PARTE 1 ✅ HECHA** (`8681636`). **Comensal + config del dueño:** carrito **line-based** (`public-menu-cart.ts`, puro+testeado — un ítem entra con distintos combos, línea agrupada por producto+opciones; precio en vivo pero el server recalcula al enviar, el carrito solo manda `option_ids`) + **picker de modificadores** (Sheet, radio si max=1 / checkbox si >1, "Agregar" habilitado solo con min/max válido) + **Sheet de revisión** (líneas+cantidad+quitar+enviar). Ítems sin modificadores conservan el stepper de Tanda C. **Config del autopedido** en `/app/mesas-qr` (`SelfOrderApi`+hooks, toggles enable+gate contra `GET/PUT /self-order/settings`). i18n ES(paridad)+EN. Gates: front build+lint+**226 tests** (+7). **PARTE 2 ✅ HECHA** (`aa6a804`): **editor de modificadores del dueño** (`modifiers-editor.tsx` + `modifiers-form.ts` puro/testeado) — botón "Opciones" en cada fila del catálogo (junto a Ficha/Receta) → Sheet CRUD de grupos (nombre+mín/máx) y opciones (nombre+precio extra), replace-all contra `PUT /products/{id}/modifiers`, pesos↔centavos, valida reglas antes del 422. **Floor:** `OrderResponse.source` → badge "Pedido del cliente" en la comanda (CUSTOMER_QR) + `selected_options` bajo cada ítem. Fixtures de `OrderDTO` con `source`. Gates: back **648**+ruff · front build+lint+**229**. **✅ CARTA QR FASE 2 COMPLETA (A–E).** Sigue **Fase 3 (pago desde la mesa)**.
