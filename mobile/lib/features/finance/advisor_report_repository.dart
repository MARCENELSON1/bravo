...
class AdvisorReport {
  ...
  factory AdvisorReport.fromJson(Map<String, dynamic> j) => AdvisorReport(
    ...
    insights: ((j['insights'] as List?) ?? const [])
        .map(
          (e) => AdvisorInsight.fromJson(Map<String, dynamic>.from(e as Map)),
        )
        .toList(),
...
final advisorReportProvider = FutureProvider.autoDispose
    .family<AdvisorReport, FinanceRange>(
      (ref, range) =>
          ref.read(advisorReportRepositoryProvider).report(rangeWindow(range)),
    );
